...
/*
 * - viewer cloud and final for rectangle
 * -s viewer cloud and final for circle
 * -f viewer final for rectangle
 * -sf viewer final for circle
 */
...
simpleVis(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud, pcl::PointCloud<pcl::PointXYZ>::ConstPtr final)
{
  // --------------------------------------------
  ...
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> originColorHandler(cloud, 255, 255, 255); // 原始点云颜色,白色
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> finalColorHandler(final, 0, 255, 0);      // 分割后的局内点颜色,绿色

  viewer->addPointCloud<pcl::PointXYZ>(cloud, originColorHandler, "sample cloud");
  viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "sample cloud"); // 点的大小
  if (final != nullptr)
  {
    viewer->addPointCloud<pcl::PointXYZ>(final, finalColorHandler, "final cloud");
...
int main(int argc, char **argv)
{
  ...
  // 填充点云数据
  if (pcl::io::loadPCDFile("/home/dxs/output.pcd", *cloud) < 0)
  {
  ...
  std::cout << "Random Sample Consensus time: " << duration.count() << " double" << std::endl;
  ...
  auto duration2 = std::chrono::duration<double>(end2 - start);
  std::cout << "visualize time: " << duration2.count() << " double" << std::endl;
