...
/*
 * -f 字段拼接 xyz（5） + rgb（5） = xyzrgb（5）
 * -p 点云拼接 xyz（3） + xyz（5） = xyz（8）
 */
#include <iostream>
...
int main(int argc, char **argv)
{
    ...
    // 打开点云文件
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("/home/dxs/output.pcd", cloud_a) == -1)
    {
        PCL_ERROR("Couldn't read file test_pcd.pcd \n");
        return (-1);
    }

    if (pcl::io::loadPCDFile<pcl::PointXYZ>("/home/dxs/output2.pcd", cloud_b) == -1)
    {
        PCL_ERROR("Couldn't read file test_pcd.pcd \n");
        return (-1);
    }
    ...
    // if (strcmp(argv[1], "-p") == 0)
        ...
        // }
        // else for (size_t i = 0; i < n_cloud_b.points.size(); ++i)
...
********************************************************************/
    ...
    std::cerr << "Cloud A size: " << cloud_a.points.size() << std::endl;
    // for (size_t i = 0; i < cloud_a.points.size(); ++i)
    // std::cerr << "    " << cloud_a.points[i].x << " " << cloud_a.points[i].y << " " << cloud_a.points[i].z << std::endl;
    // 输出Cloud B
    std::cerr << "Cloud B: " << std::endl;
    if (strcmp(argv[1], "-p") == 0)
        std::cerr << "Cloud B size: " << cloud_b.points.size() << std::endl;
    // for (size_t i = 0; i < cloud_b.points.size(); ++i)
    ...
    if (strcmp(argv[1], "-p") == 0)
    {
        ...
        std::cerr << "Cloud C size: " << cloud_c.points.size() << std::endl;
    ...
    pcl::io::savePCDFileASCII("../saved.pcd", cloud_c);
    return (0);
}
